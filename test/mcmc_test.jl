@testitem "fixed discretization" tags = [:mcmc] begin
    ...
    respD = [
        MTResponseDistribution(normal_dist, normal_dist),
        SurfaceWaveResponseDistribution(normal_dist),
        SurfaceWaveResponseDistribution(normal_dist),
    ]

    true_models = [
        (; m = [2.0, 1.0, 2.0], h = [1000.0, 1000.0]),
        (;
            m = [3500.0, 3000.0, 3500.0] ./ 1e3,
            h = [100.0, 100.0],
            vp = [7000.0, 7000.0, 7000.0] ./ 1e3,
            ρ = [2500.0, 2500.0] ./ 1e3,
        ),
        (;
            m = [3500.0, 3000.0, 3500.0] ./ 1e3,
    ...
    @testset "$(model_types[ik]) : $(sampler_)" for ik in eachindex(model_types),
        ...
        for idx = 1:n_samples
            ...
            err[idx] = χ²(
                ...
                reduce(vcat, [getfield(resp, k) for k in propertynames(resp)]);
                W = W,
            )
        end
        @show err
