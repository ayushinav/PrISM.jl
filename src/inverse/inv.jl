...
function inverse!(mₖ::model1,
    ...
    prep_j = prepare_jacobian(
        wrapper_DI!, rvec, ad_type, mₖ.m, Cache_DI(m_cache), Constant_DI(const_m),
        Cache_DI(resp_cache), Constant_DI(vars), Constant_DI(response_fields),
        Constant_DI(model_type), Constant_DI(model_trans_utils),
        Constant_DI(response_trans_utils), Constant_DI(params))

    DifferentiationInterface.jacobian!(wrapper_DI!, rvec, jc, prep_j, ad_type, mₖ.m,
