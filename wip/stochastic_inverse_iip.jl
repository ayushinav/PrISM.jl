...
"""
function stochastic_inverse3(r_obs::resp1, err_resp::resp2, vars, alg_cache::mcmc_cache;
    ...
    #     [Array{ForwardDiff.Dual, ndims(randmat)}copy((getproperty(r_obs, k)))
    #     for k in propertynames(r_obs)]
    # )

    resp_sample = copy(r_obs)
...
@model function mcmc_turing2(m_sample::model,
    ...
    forward!(r_sample, m_sample, vars, response_trans_utils)
    r2 = fwd_wrapper(r_sample, m_sample, vars, response_trans_utils)

    for k in response_fields
        r_obs[k] ~ getfield(rDist, k)(getfield(r2, k), getfield(err_resp, k) .^ 2)
    end
end
